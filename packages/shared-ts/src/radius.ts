...
function assertSteps(steps: number): void {
  if (!Number.isInteger(steps) || steps < 4) {
    throw new Error("Circle steps must be an integer of at least 4.");
  }
}
...
/** Returns whether a point is on or within a radius around a center point. */
export function pointWithinRadius(
  point: Position,
  center: Position,
  radiusMeters: number,
): boolean {
  assertPosition(point, "Point");
  assertPosition(center, "Center");
  assertRadius(radiusMeters);
...
/**
 * Produces a closed GeoJSON polygon approximating a geodesic distance circle.
 * The default 96 vertices keeps a 40km overlay smooth without excessive map
 * source updates.
 */
export function circlePolygon(
  ...
  return {
    ...
    geometry: {
      type: "Polygon",
      coordinates: [circleRing(center, radiusMeters, steps)],
    },
  };
}
